...
def has_any_neighbour(coords, all_coords):
    ...
    '''
    return has_up_neighbour(coords, all_coords) \
...
def get_manhattan_distance(coords_a, coords_b): 
    ...
    '''
    return get_distance(coords_a[0], coords_b[0]) + get_distance(coords_a[1], coords_b[1]) 
...
def get_min_manhattan_distance(coords, all_coords): 
    ...
    '''
    return min([get_manhattan_distance(coords, another_coords)for another_coords in all_coords])
...
def is_bot_left_corner(floor_coords, all_floor_coords):
    ...
    '''
    return has_up_neighbour(floor_coords, all_floor_coords) \
...
def is_top_left_corner(floor_coords, all_floor_coords):
    ...
    '''
    return has_right_neighbour(floor_coords, all_floor_coords) \
...
def is_top_right_corner(floor_coords, all_floor_coords):
    ...
    '''
    return has_left_neighbour(floor_coords, all_floor_coords) \
           and has_down_neighbour(floor_coords, all_floor_coords) \
           and not has_right_neighbour(floor_coords, all_floor_coords) \
           and not has_up_neighbour(floor_coords, all_floor_coords)
...
def is_corner(floor_coords, all_floor_coords):
    ...
    '''
    return is_bot_left_corner(floor_coords, all_floor_coords) \
...
def get_floor_area_corners(all_floor_coords): 
    ...
    '''
    return {floor_coords for floor_coords in all_floor_coords if is_corner(floor_coords, all_floor_coords)}
...
def taboo_cells_coords(warehouse):
    ...
    rule_1 = {cell for cell in get_floor_area_corners(floor_coords) if cell not in warehouse.targets}
    ...
    for corner_combination in itertools.combinations(rule_1, 2):
        ...
        for space in space_in_line(cell_a, cell_b):
            # Check Rule 2 fails
            if(space in warehouse.targets) or not(space in floor_coords and has_any_neighbour(space, warehouse.walls)):
...
class SokobanPuzzle(search.Problem):
    ...
    def h(self, node):
        ...
        for perm in itertools.permutations([i for i in range(len(boxes))]):
            ...
            perm_boxes_term = sum([get_manhattan_distance(boxes[next(counter)], targets[j]) + (self.box_weight_map[boxes[next(counter_2)]]) for j in perm]) 
...
def solve_weighted_sokoban(warehouse):
    ...
    elif(goal_node is None or check_elem_action_seq(warehouse, trace_actions(goal_node)) == "Impossible"):
        return ["impossible", None]
    # Solution is found
    else:
